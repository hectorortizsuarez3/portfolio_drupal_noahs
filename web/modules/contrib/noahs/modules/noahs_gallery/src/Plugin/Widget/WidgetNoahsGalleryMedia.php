 ...
 * )
 */
class WidgetNoahsGalleryMedia extends WidgetBase {
  ...
  public function buildWidgetForm (array $form) {
    ...
    $galleries = \Drupal::entityTypeManager()->getStorage('noahs_gallery_type')->loadByProperties(['status' => TRUE]);

    $opciones = [];
    $opciones[''] = t('Select a gallery');
    foreach ($galleries as $gallery) {
      $opciones[$gallery->id()] = $gallery->get('label')->value;
    }
    ...
    $form['noahs_gallery_entity'] = [
      'type'    => 'select',
      'title'   => t('Noahs Gallery Entity'),
      'tab' => 'section_content',
      'options' => $opciones,
      'attributes' => [
  ...
  public function template($settings) {
    ...
    $image_style = !empty($settings->element->gallery_image_style) ? $settings->element->gallery_image_style : 'Noahs 1024x768';
    ...
    if (!empty($settings->element->gallery_items)) {
      $items = $settings->element->gallery_items;
    }
    elseif (!empty($settings->element->noahs_gallery_entity)) {
      $gallery = \Drupal::entityTypeManager()->getStorage('noahs_gallery_type')->load($settings->element->noahs_gallery_entity);
      $items = $gallery->get('images')->referencedEntities();
    }
    ...
    if (!empty($items)) {
      foreach ($items as $i => $item) {
        ...
        if (!empty($settings->element->noahs_gallery_entity)) {
          $mid = $item->mid->value;
        }
        ...
        if (isset($item->url)) {
          ...
          if (!empty($item->node_id)) {
            $url = \Drupal::service('path_alias.manager')->getAliasByPath('/node/' . $item->node_id);
          }
        ...
        if (!$remove_fancybox) {
          $output .= '<a data-fancybox="gallery" href="' . $image . '"><i class="fa-solid fa-magnifying-glass"></i></a>';
        }
