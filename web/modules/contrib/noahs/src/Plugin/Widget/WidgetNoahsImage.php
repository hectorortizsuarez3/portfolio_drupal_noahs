 ...
 * )
 */
class WidgetNoahsImage extends WidgetBase {
      ...
      'icon' => '<svg version="1.1" id="fi_149092" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 58 58" style="enable-background:new 0 0 58 58;" xml:space="preserve">
            <g>
               ...
               <path d="M7,46c0.234,0,0.47-0.082,0.66-0.249l16.313-14.362l10.302,10.301c0.391,0.391,1.023,0.391,1.414,0s0.391-1.023,0-1.414
                  ...
                  c-0.374-0.373-0.972-0.392-1.368-0.044L6.339,44.249c-0.415,0.365-0.455,0.997-0.09,1.412C6.447,45.886,6.723,46,7,46z"></path></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g><g></g></svg>',
      'title' => 'Image',
  'description' => 'Display an image with custom styles and alignment.',
      'group' => 'primary',
    ];
  }
  ...
  public function buildWidgetForm(array $form) {
    ...
    $form['inner_horizontal_align'] = [
      'type'    => 'select',
      'title'   => t('Inner horizontal align'),
      'tab' => 'section_content',
      'style_type' => 'style',
      'style_selector' => '.widget-image-wrapper',
      'style_css' => 'justify-content',
  ...
  public function template($settings) {
    ...
    $style = NULL;
    $url = NULL;
    $alt = !empty($settings->image_alt->text) ? $settings->image_alt->text : '';
    $title = !empty($settings->image_title->text) ? $settings->image_title->text : '';

    if (!empty($settings->image->image_style)) {
      $style = $settings->image->image_style;
    }
    ...
    if (!empty($settings->image_url->node_id)) {
      $url = $this->aliasManager->getAliasByPath('/node/' . $settings->image_url->node_id);
    }
    elseif (!empty($settings->image_url->text)) {
      // Buscar un número entre paréntesis en el texto
      if (preg_match('/\((\d+)\)/', $settings->image_url->text, $matches)) {
        $node_id = $matches[1];
        $url = $this->aliasManager->getAliasByPath('/node/' . $node_id);
      }
      else {
        $url = $settings->image_url->text;
      }
    }
    if (!empty($settings->image_url->url_params)) {
      $url = $url . $settings->image_url->url_params;
    }
    $mid = !empty($settings->image->fid) ? $settings->image->fid : NULL;

    $html = '<div class="widget-image-wrapper noahs-contextual">';
    $html .= $this->getMediaImage($mid, $style, null, $alt, $title, $url);
    $html .= '</div>';
